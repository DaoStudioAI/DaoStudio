...
public class OllamaToolCallTests : IDisposable
{
    ...
    {
        // First yield a human-readable text fragment the assistant might produce
        var textUpdate = new ChatResponseUpdate(ChatRole.Assistant, "list the files");
        yield return textUpdate;

        // Then yield the content update that carries the function call
        var contentUpdate = new ChatResponseUpdate(ChatRole.Assistant, new List<AIContent> { toolCall });
